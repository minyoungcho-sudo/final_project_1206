...
import { renderCommunityPage, setupCommunityPage } from './pages/CommunityPage.js'
import { renderAdminCommunityPage, setupAdminCommunityPage } from './pages/AdminCommunityPage.js'
...
function render() {
  ...
  switch (window.appState.currentPage) {
    ...
    case 'myactivity':
      ...
      break;
    case 'community':
      app.innerHTML = renderCommunityPage();
      setupCommunityPage();
      break;
    case 'admin-community':
      // 관리자 커뮤니티 페이지 접근 권한 확인
      const adminCommunityUserId = localStorage.getItem('currentUserId');
      const adminCommunityUserRole = localStorage.getItem('userRole');
      
      if (adminCommunityUserRole !== 'teacher' || !adminCommunityUserId || !isAdminUser(adminCommunityUserId)) {
        app.innerHTML = `
          <div class="min-h-screen bg-gray-50 dark:bg-gray-900 flex items-center justify-center p-4">
            <div class="max-w-md w-full bg-white dark:bg-gray-800 rounded-xl shadow-lg p-6 text-center">
              <span class="material-symbols-outlined text-red-600 dark:text-red-400 text-6xl mb-4">block</span>
        ...
        app.innerHTML = renderAdminCommunityPage();
        setupAdminCommunityPage();
      }
      break;
    default:
      app.innerHTML = renderInputPage();
